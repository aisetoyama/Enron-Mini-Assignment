...
def email_analyse(inputfile, dictionary_list, to_email_dict):
    ...
    if email['to']:
        ...
        for email_to_1 in email_to:
            to_email_list.append({"msg_id": email['Message-ID'], 
...
#     print(subdirectory)
    for filename in filenames:
        if filename != "categories.txt" and filename.endswith(".txt"):
            email_analyse(os.path.join(directory, filename), dictionary_list, to_email_list)
...
mycursor.executemany(sql1, dictionary_list)
mycursor.executemany(sql2, to_email_list)
